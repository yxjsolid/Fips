...
int ECCOnePassUnifiedMain(int argc, char **argv)
{
	char **args = argv + 1;
	int argn = argc - 1;
	FILE *in, *out;
...
//		printf("keyworkd = %s \n", keyword);
		...
		else if (!strcmp(keyword, "CAVSTag"))
		{
			...
			kavasHashZ(group, Ze, Zlen, cd, scx, scy, eid, six, siy);
			kavasHashZ(group, Zs, Zlen, cd, scx, scy, sid, six, siy);
...
//	testHmac();
	...
	ECCOnePassUnifiedMain(argc, argv);
}
